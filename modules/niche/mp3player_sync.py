...
def sync_to_player(playlist_inputs, output_dir, playlist_files_mode=False):
    """Sync playlists to MP3 player with format conversion.
    
    Converts audio to 320kbps MP3 and resizes album art to 600x600 JPG.
    
    Args:
        playlist_inputs: List of playlist directory paths or playlist file paths
        output_dir: Output directory (MP3 player location)
    ...
    print(f"Syncing playlists to MP3 player: {output_dir}")
    print(f"Mode: {mode_str}")
    print(f"Format: MP3 320kbps, Album art: 600x600 JPG")
    print("-" * 60)
...
def main():
    """Main entry point for MP3 player sync tool."""
    parser = argparse.ArgumentParser(
        description='Sync playlists to MP3 player with 320kbps MP3 conversion',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Converts audio files to 320kbps MP3 and resizes album art to 600x600 JPG
while copying playlists and their files to the MP3 player.

Examples:
  # Sync playlist directories (default)
  walrio mp3player_sync /path/to/playlists /media/mp3player

  # Sync multiple playlist directories
  walrio mp3player_sync /path/to/playlists1 /path/to/playlists2 /media/mp3player
  
  # Sync individual playlist files
  walrio mp3player_sync --playlist-files favorites.m3u workout.m3u /media/mp3player
"""
    )
    parser.add_argument(
        'playlist_inputs',
