...
def setup_additional_files(dist_dir):
    ...
    for plugin_file in plugins_src.glob(plugin_extension):
        ...
        if any(essential in plugin_name.lower() for essential in [
            ...
            "gstaudiotestsrc", "gstvolume", "gstalsa", "gstpulseaudio", "gstpipewire", "gstautodetect",
...
def build_pyinstaller_command(gui_name, entry_point, debug=False):
    ...
    # Add spec file parameter to filter out problematic plugins
    cmd.extend(["--additional-hooks-dir", str(Path(".github/scripts").absolute())])
